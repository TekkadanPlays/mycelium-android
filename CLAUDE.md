...
- **ViewModels** (`viewmodel/`, 12 files): Activity-scoped globals (`AppViewModel`, `AccountStateViewModel` [2,427 lines], `FeedStateViewModel`) and NavBackStackEntry-scoped per-screen VMs (`DashboardViewModel`, `Kind1RepliesViewModel`, `ThreadRepliesViewModel`, `TopicsViewModel`, `RelayManagementViewModel` (bridges `RelayHealthTracker` + `RelayDeliveryTracker` for inline relay health display), `SearchViewModel`, `AnnouncementsViewModel`, `AuthViewModel`, `ThreadStateHolder`)
...
- **Startup:** `StartupOrchestrator` coordinates 6 phases: Settings (CRITICAL, kind-30078 on **outbox-first** relay list, concurrent with Phase 1) + User State (HIGH) → Feed (HIGH) → Enrichment (NORMAL) → Background (LOW). **Deep history** starts ~5s after Phase 1 (LOW priority batches, overlapping feed). The feed waits on `userStateReady` and up to ~9s on `settingsReady` so NIP-78 defaults apply before the main REQ. Onboarding may apply kind-30078 after NIP-65 save and `markSettingsApplied` so Phase 0 skips. **Health UI:** `connectionAttempts` counts lifetime handshakes (one multiplexed WebSocket per relay URL in `CybinRelayPool`).
